...
void loop() {
  ...
  if (max_result > 0.8f) {
    const char* detected = kCategoryLabels[max_idx];
    MicroPrintf("Detected %7s, score: %.2f", detected,
                static_cast<double>(max_result));
    // Notify sensor task
    if (strcmp(detected, "on") == 0) {
      xEventGroupSetBits(voice_events, EVT_ON);
    } else if (strcmp(detected, "off") == 0) {
      xEventGroupSetBits(voice_events, EVT_OFF);
    }
  }
#else
